  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        ...
        break
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user ```ahora es admin supremo```')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```ya no es admin supremo```')
